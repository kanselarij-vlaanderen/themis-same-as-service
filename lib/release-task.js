...
import { MU_APPLICATION_GRAPH, SAME_AS_GRAPH } from '../config';
...
class ReleaseTask {
  ...
  */
  ...
  async renameUris() {
    const triples = await getGraphTriples(this.source);
    const renamedTriples = await renameTriples(triples);

    await this.updateTriples(renamedTriples);
  }
